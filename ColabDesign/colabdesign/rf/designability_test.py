...
# map fixed residues in the new designed protein using the traceback file
def map_and_transform_contig(data):
    output = []  
    pointer = 0
    for item in data:
        parts = item.split('/')
        ...
        for part in parts:            
            if part[0] in string.ascii_uppercase:
                ...
                split_start = re.findall(r'[A-Za-z]+|\d+', start)
                start_num = int(split_start[1])                
                end_num = int(end)                
                length = end_num - start_num
                trans_start = pointer+1
                trans_end = trans_start+length
                
                pointer = trans_end
                sequence.append(f'{letter}{trans_start}-{trans_end}')
            else:
                start, end = part.split('-')
                pointer = pointer+int(end)
...
def main(argv):
  ...
  ag.add(["trb="          ],  None,   str, ["input trb file path"])
  ag.txt("-------------------------------------------------------------------------------------")
  ag.txt("OPTIONAL")
  ag.txt("-------------------------------------------------------------------------------------")
  ...
  for m in range(o.num_designs):
    ...
    # Code for trb processing inspired by addFIXEDlabels.py code in https://github.com/nrbennet/dl_binder_design/tree/main/af2_initial_guess
    trb_path = os.path.join(o.trb, f'{pdb_filename.split(".")[0]}.trb')
    print("trb file =",trb_path)
    pdbs.append(pdb_filename)
    if not os.path.exists(trb_path):
        print(f"Error: TRB file not found for PDB {pdb_filename}")
        continue
    
    data = np.load(trb_path, allow_pickle=True)

    # Identify the corresponding fixed residues as array in the re-designed protein
    samp_mask = data['sampled_mask']
    o.contigs = map_and_transform_contig(samp_mask)
  ...
  with open(f"{o.loc}/design.fasta","w") as fasta:
    for m,(out,pdb_filename) in enumerate(zip(outs,pdbs)):
      ...
      fin_data += [[out[k][n] for k in labels] for n in range(o.num_seqs)]
      print("$$$$",fin_data)
  ...
  labels[2] = "mpnn"
  df = pd.DataFrame(fin_data, columns=labels)
